    ...
    with open(args.testfilepath + args.alias + 'test.sql' + '.csv', 'r') as f:  # modify
        ...
        for query in queries:
            ...
            for est in estimators:
                ...
                est.AddError(err, est_card, true_card)
...
def SaveEstimators(path, estimators, return_df=False, result_path=f'./{os.path.splitext(args.csvname)[0]}.naru.txt'):
    ...
    for est in estimators:
        ...
        est.report()
        qerror = np.array(est.errs)
        
        fmetric.write(f"est.name: {est.name}\n")
        for i, percentile in enumerate([50, 90, 95, 99, 100]):
            fmetric.write(f"Q-Error {percentile}%-Percentile: {np.percentile(est.errs, percentile)}\n")
        fmetric.write(f"Q-Mean wo inf {np.mean(qerror[np.isfinite(qerror)])}\n")
        fmetric.write(f"Latency avg: {np.mean(est.query_dur_ms):.2f}ms\n")
        fmetric.close()
    if return_df:
        return results

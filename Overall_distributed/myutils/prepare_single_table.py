"""参考deepdb的prepare_single_table.py"""
import numpy as np
import pandas as pd
import logging
import os
...
def prepare_single_table(schema_graph, table, path, max_distinct_vals=100000, csv_seperator=',', header=None,
    ...
    if os.path.exists(path):
            logger.info(f"Removing target path {path}")
            shutil.rmtree(path)

    logger.info(f"Making target path {path}")
    os.makedirs(path)
    ...
    print("relevant_attributes: ", relevant_attributes)
    ...
    if len(table_data) > max_table_data:
        table_data = table_data.sample(max_table_data)

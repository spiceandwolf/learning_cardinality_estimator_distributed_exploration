...
### 0.实验准备
 ...
 $\color{green}{基于学习的ai分布式基数估计器实现目标 : 在单机与分布式环境下都可以正常工作的学习型基数估计器}$
...
### 2.模拟一般分布式数据库的数据划分，query只在单表上进行
  ...
  ***当前问题无法解决，实验暂停，直接开始设计学习型分布式基数估计器的模型 2023/03/15左右***  
  在分布式数据库中各节点的特点根据数据的分布状态划分出具有不同特点的集合？  
      <Pi, Pj,...> : 由某一关系表R(子关系/子表以后也需要考虑)被根据行划分后得到，具有同样的属性，合并到一起后可以得到原来的表R。影响某属性的数据分布？  
      [Pi, Pj,...] : 由某一关系表R(子关系以后也需要考虑)被根据列划分后得到，具有的属性不完全相同，所有属性的交集应该是原来的表R主键(属于哪种约束等级？)。影响对属性间的相关性计算？  
      {Pi, Pj,...} : 可能由上面两种节点集合被进一步处理后得到的节点组成，合并到一起后得到的表r可能只是一个子表。具有前两种节点集合的所有影响？  

  样本空间Ω被划分成若干个子样本空间{$Ω_1$, $Ω_2$, $Ω_3$,...}, 在样本空间Ω上的密度函数和在子样本空间中的密度函数有什么关系？  

  在分布式数据库中，各个节点中基数估计所求的选择率都是原始定义域中对应基数估计的条件概率，先验信息是划分过程所用的函数。在各节点中采样并建模得到模型(指直方图、ml模型等)，是属于该节点的条件概率密度函数(或条件概率分布函数)，涉及多个表(分布式节点中的表，不一定等于数据库的表)的join的基数估计问题会涉及求相关节点的联合概率密度(或联合分布)。***分布式基数估计和集中式基数估计的主要区别是，前者是从条件概率出发求联合概率(全部属性的子集)，后者是从联合概率(全部属性)出发求条件概率***  

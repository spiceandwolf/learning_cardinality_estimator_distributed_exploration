...
if method == 'naru':
    ...
    os.system(
        'python train_model.py --version ' + version + ' --num-gpus=1 --dataset=dmv --epochs=70 --warmups=8000 --bs=2048 --residual --layers=5 --fc-hiddens=256 --direct-io ') 
    
    os.system(
        'python eval_model.py --testfilepath ../sql_truecard/ --version ' + version + ' --table ' + version + ' --alias cdcs --dataset=dmv --glob=\'<ckpt from above>\' --num-queries=1000 --residual --layers=5 --fc-hiddens=256 --direct-io ')
    # ai4db
    os.chdir('..')

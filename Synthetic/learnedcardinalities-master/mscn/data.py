...
# ai4db
#  每个编码只对一个predict编码，其他不变
def make_dataset_distributed(samples, predicates, joins, labels, max_num_joins, max_num_predicates):
    # 每个子查询都要作为一个完整的查询处理，所以一个查询原本有多少个子查询就要扩增为多少条编码
    sample_masks = []
    sample_tensors = []
    predicate_masks = []
    predicate_tensors = []
    ...
    for sample, predicate, join, label in zip(samples, predicates, joins, labels):
        # 编码sample部分
        sample_tensor = np.vstack(sample)
        num_pad = max_num_joins + 1 - sample_tensor.shape[0]
        sample_mask = np.ones_like(sample_tensor).mean(1, keepdims=True)
        ...
        predicate_tensor = np.vstack(predicate)
        num_pad = max_num_predicates - predicate_tensor.shape[0]
        # print('predicate: ', predicate_tensor.shape)
        for sub_predicate in predicate:
            sub_predicate_expand = []
            sub_predicate_expand.append(sub_predicate)
            infilling = np.zeros_like(sub_predicate)
            for i in range(len(predicate) - 1):
                sub_predicate_expand.append(infilling)
            sub_predicate_tensor = np.vstack(sub_predicate_expand)
            # print('sub_predicate_expand: ',sub_predicate_tensor.shape)
            sub_predicate_mask = np.ones_like(sub_predicate_tensor).mean(1, keepdims=True)
            # print('sub_predicate_mask:', sub_predicate_mask.shape)
            sub_predicate_tensor = np.pad(sub_predicate_tensor, ((0, num_pad), (0, 0)), 'constant')
            sub_predicate_mask = np.pad(sub_predicate_mask, ((0, num_pad), (0, 0)), 'constant')
            predicate_tensors.append(np.expand_dims(sub_predicate_tensor, 0))
            predicate_masks.append(np.expand_dims(sub_predicate_mask, 0))

            # 每个sub_predicate对应的sample，join和label都和原本的predicate一样
            sample_tensors.append(np.expand_dims(sample_tensor, 0))
            sample_masks.append(np.expand_dims(sample_mask, 0))
            join_tensors.append(np.expand_dims(join_tensor, 0))
            join_masks.append(np.expand_dims(join_mask, 0))
    ...
    target_tensor = torch.FloatTensor(target_tensors)

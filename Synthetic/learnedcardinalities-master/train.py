...
import torch
from mscn.data import get_train_datasets, load_data, make_dataset, make_dataset_distributed
...
def train_and_predict(train_file, test_file, num_queries, num_epochs, batch_size, hid_units, cuda, need_train=True):
    ...
    if need_train:
        ...
        test_data_distributed = make_dataset_distributed(samples_test, predicates_test, joins_test, labels_test, max_num_joins,
                                 max_num_predicates)
        test_data_distributed_loader = DataLoader(test_data_distributed, batch_size=batch_size)

        preds_test_distributed, t_total_distributed = predict(model, test_data_distributed_loader, cuda)

        # Unnormalize
        preds_distributed_test_unnorm = unnormalize_labels(preds_test_distributed, min_val, max_val)
        ...
        for predicate in predicates:
            n += len(predicate)
            predicate_counts.append(len(predicate))
        # print('n :', n)
        # print('len(preds_distributed_test_unnorm): ', len(preds_distributed_test_unnorm))
        file_name = test_file + ".mscn.result.distributed.csv"
        os.makedirs(os.path.dirname(file_name), exist_ok=True)
        with open(file_name, "w") as f:
            n = 0
            for i in range(len(label)):
                for j in range(predicate_counts[i]):
                    f.write(str(preds_distributed_test_unnorm[n]) + "," + label[i] + "\n")
                    n += 1
        f.close()  # remark
        # ai4db

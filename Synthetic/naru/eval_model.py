...
os.environ['CUDA_VISIBLE_DEVICES'] = '0' # ai4db modified
...
def preprocess_sql(sql_path):
    ...
    for col in cols:
        subqueries = []
        distributed_sql_path = args.testfilepath + version + 'test_' + col + '.sql'
        with open(distributed_sql_path, 'r') as f:
            for line in f.readlines():
                # print (line)
                sql = ','.join(line.split(',')[:-1])
                cardinality = line.split(',')[-1].strip('\n')
                ...
                for cond in conditions:
                    ...
                    conds.append(right)
                # print (tables, joins, conds, cardinality)
                subqueries.append(','.join(tables) + '#' + ','.join(joins) + '#' + ','.join(conds) + '#' + cardinality)
        with open(distributed_sql_path + '.csv', 'w') as f:
...
def RunN(table,
    ...
    with open(args.testfilepath + version + 'test.sql' + '.csv', 'r') as f:  # modify
        ...
        queries = f.readlines()[0:1000] # ai4db modified 为了和其他基数估计器的测试方案保持一致，改为查询1000条
        testt1 = time.time()
        for query in queries:
            # print(query) # ai4db modified
            ...
            estcard = estimators[0].Query(cols, ops,
                                          vals)  # oracle_est
            # print('res:', estcard, true_card) # ai4db modified
    ...
    # ai4db
    # 处理分布式子查询
    csv_distributed_naru_results = []
    distributed_sub_cardinality_predict_lists = []
    for col in cols:
        # 读取每个子查询对应的csv文件
        distributed_results = []
        with open(args.testfilepath + version + 'test_cdcs.' + col.name + '.sql.csv', 'r') as f: # 注意这里图方便直接写死了别名
            queries = f.readlines()[0:1000]  # .Parsing
            idx = 0
            for query in queries:
                # print(query) # ai4db modified
                tables = [x.split(' ')[0] for x in query.split('#')[0].split(',')]
                ...
                key = '_'.join(sorted(alias))
                conditions = query.split('#')[2].split(',')
                true_card = int(a[idx]) # 需要的是完整sql真实的基数
                idx = idx + 1
                columns = np.asarray(table.columns)
                # print('columns',columns)
                names = [c.name for c in columns]
                ...
                for i in range(int(len(conditions) / 3)):
                    ...
                    if op == '<':
                        ...
                        if position >= 0:
                            val = x[position]
                    # print (f'Updated:{op}, {val}')
                    distributed_cols.append(col)
                    ops.append(op)
                    vals.append(val)
                
                query = distributed_cols, ops, vals
                estcard = estimators[0].Query(distributed_cols, ops, vals)  # oracle_est
                # print('res:', estcard, true_card) # ai4db modified
                distributed_results.append({'cardinality_predict' : estcard, 'cardinality_true' : true_card})
        distributed_sub_cardinality_predict_lists.append(distributed_results)   
    ...
    for i in range(len(distributed_sub_cardinality_predict_lists)):
        for j in range(len(distributed_sub_cardinality_predict_lists[i])):
            csv_distributed_naru_results.append(distributed_sub_cardinality_predict_lists[i][j])
    # 保存预测结果
    import csv
    with open(args.testfilepath + version + 'test.sql.naru.result.distributed.csv', 'w', newline='') as f:
        w = csv.DictWriter(f, csv_distributed_naru_results[0].keys())
        for row in csv_distributed_naru_results:
            w.writerow(row)
    # ai4db
    return False
...
def Main():
    ...
    # SaveEstimators(args.err_csv, estimators) # ai4db modified 该函数里有bug
